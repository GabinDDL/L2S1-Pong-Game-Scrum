...
*/
public class BotPongController implements Initializable {
    
    // Elements created and described in classicPongMenu.fxml
    @FXML
    BorderPane botPongMenuContainer;
    @FXML
    ImageView leftArrow;
    @FXML
    ...
    Text title;
    @FXML
    ChoiceBox<String> playerLeft;
    @FXML
    ChoiceBox<String> playerRight;
    @FXML
	...
	public void initialize(URL url, ResourceBundle rb) {
        ...
        // We defin buttons that allow us to restart or quit game to menu
    ...
    public void loadHomePage(MouseEvent event) throws IOException {
        ...
        TransitionClass.transition(false, false, "home.fxml", Interpolator.EASE_IN, botPongMenuContainer, null);
    }
    ...
    public void loadMenu(KeyEvent event) throws IOException {
        ...
        TransitionClass.transition(false, false, "botPongMenu.fxml", Interpolator.EASE_IN, (Parent) gameplay, new EndFunction() {
            public void end() {
                sceneDisplayModifier.pauseUnpause();
                inReset = true;
            }
    ...
    public void loadClassicPage(MouseEvent event) throws IOException {
        ...
        TransitionClass.transition(true, false, "classicPongMenu.fxml", Interpolator.EASE_IN, botPongMenuContainer, null);
    }
    ...
    public void playGame(MouseEvent event) throws IOException {
        ...
        Scene gameScene = botPongMenuContainer.getScene();
        ...
        switch ((String) playerLeft.getValue()) {
            case "JOUEUR": 
                playerA = new Player();
                break;
            case "EASY":
                playerA = new Bot(Difficulty.EASY);
                break;
            case "HARD":
                playerA = new Bot(Difficulty.HARD);
                break;
            case "NORMAL":
            default:
                playerA = new Bot(Difficulty.NORMAL);
                break;
        }

        playerA.setScore(scoreA);

        // Exchange the next two lines to interchange bot and player on the right player.
        // Player playerB = new Player();
        Player playerB;

        switch ((String) playerRight.getValue()) {
            case "EASY":
                playerB = new Bot(Difficulty.EASY);
                break;
            case "HARD":
                playerB = new Bot(Difficulty.HARD);
                break;
            case "NORMAL":
            default:
                playerB = new Bot(Difficulty.NORMAL);
                break;
        }
        ...
        TransitionClass.classicGameTransition(false, true, Interpolator.EASE_IN, botPongMenuContainer, gameplay, new EndFunction() {
            public void end() {
                launchTimer(gameplay, court, gameView, 3, new EndFunction() {
                    public void end() {
                        gameView.animate();
